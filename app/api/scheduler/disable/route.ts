...
export async function POST(request: NextRequest) {
  try {
    ...
    const scheduler = await schedulerService.disableScheduler(id, email);
    return NextResponse.json({ success: true, data: scheduler });
  } catch (error) {
    console.error('POST /api/scheduler/disable error:', error);
    const message = error instanceof Error ? error.message : 'Gagal menonaktifkan scheduler.';
