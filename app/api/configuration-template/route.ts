...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await configurationTemplateService.getTemplates(params);
    return NextResponse.json({ success: true, ...result });
  } catch (error) {
    console.error('GET /api/configuration-template error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const template = await configurationTemplateService.createTemplate(body);
